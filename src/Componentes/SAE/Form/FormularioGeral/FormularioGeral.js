...
function FormularioGeral(props) {
  ...
  React.useEffect(() => {
    axios
      .post(
        '1222.44://sistema.cssantos.com.br/api/API_SAE.php?form=' + display + ''
      )
      .then((resp) => {
        setCamposGerais(resp.data);
      })
      .catch((ERR) => {
        ...
        <>
          ...
          {Object.keys(camposGerais).map((index, item) => (
            <div
              ...
              className={
                'linha grid_' +
                btoa(index).replaceAll('=', '').replaceAll('+', '') +
                ' ' +
                index
              }
            >
...
function BoxDoForm(props) {
  ...
  return (
    <div className={'bloco' + titulo + ' bloco ' + display + ''}>
      <h2>{titulo}</h2>
      {/*console.log("\n \r")*/}
      {dados.map((item, index) => (
        <RenderizaCamposGerais key={index} funcao={props.funcao} dados={item} />
      ))}
    </div>
  );
}
