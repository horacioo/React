...
function Form(props) {
    ...
    function enviarNoSubmit(e) {
        ...
        /*axios.get('http://sistema.cssantos.com.br/api/API_SAE.php?RecebeCampos='+ btoa(JSON.stringify(data))+'').then().catch(ERR => { console.log(ERR.message); })*/
        ...
        fetch("http://sistema.cssantos.com.br/api/API_SAE.php",
        { method: "POST", body: datax  },[]).then(function(res){ res.json(); }).then(function(myJson) { console.log(myJson); setDataDeInsercao(myJson) }); 
        /******************************************************/
        ...
        <>
            ...
            <form action='' method="post" onSubmit={enviarNoSubmit}>
                <div id='formularioCompleto' >
                    ...
                    {(props.sexo === "F") ? <><FormularioGeral funcao={funcaoGetData} dysplay='Feminino' /></> : <><FormularioGeral funcao={funcaoGetData} dysplay='Masculino' /></>}
                </div>
